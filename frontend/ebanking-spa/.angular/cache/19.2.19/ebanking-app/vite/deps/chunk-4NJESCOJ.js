  ...
  */
  watchFragment(options) {
    ...
    if (__DEV__) {
      ...
      if (!id) {
        __DEV__ && invariant3.warn(110, actualFragmentName);
      }
    }
...
var EntityStore = class {
  ...
  merge(older, newer) {
    ...
    if (!incoming) return;
    invariant3(typeof dataId === "string", 96);
              ...
              if (__DEV__) {
                const checkReference = (ref) => {
                  if (this.lookup(ref.__ref) === void 0) {
                    __DEV__ && invariant3.warn(97, ref);
                    return true;
                  }
                };
                if (isReference(newValue)) {
                  ...
                  for (const value of newValue) {
                    ...
                    if (seenReference && someNonReference !== void 0) {
                      __DEV__ && invariant3.warn(98, someNonReference);
                      break;
                    }
                  }
                }
...
function keyFieldsFnFromSpecifier(specifier) {
  ...
  return info.keyFieldsFn || (info.keyFieldsFn = (object, context) => {
    ...
    const keyObject = context.keyObject = collectSpecifierPaths(specifier, (schemaKeyPath) => {
      ...
      invariant3(extracted !== void 0, 99, schemaKeyPath.join("."), object);
      return extracted;
    });
...
var Policies = class {
  ...
  setRootTypename(which, typename = which) {
    ...
    if (typename !== old) {
      invariant3(!old || old === which, 100, which);
        ...
        if (supertypeSet.has(supertype)) {
          if (!typenameSupertypeSet.has(supertype)) {
            if (checkingFuzzySubtypes) {
              __DEV__ && invariant3.warn(101, typename, supertype);
            }
...
function normalizeReadFieldOptions(readFieldArgs, objectOrReference, variables) {
  ...
  if (__DEV__ && options.from === void 0) {
    __DEV__ && invariant3.warn(102, stringifyForDisplay(Array.from(readFieldArgs)));
  }
...
function assertSelectionSetForIdValue(store, field, fieldValue) {
  if (!field.selectionSet) {
    ...
    workSet.forEach((value) => {
      if (isNonNullObject(value)) {
        invariant3(!isReference(value), 105, getTypenameFromStoreObject(store, value), field.name.value);
  ...
  processSelectionSet({
    ...
    this.flattenFields(
      ...
      !policies.getReadFunction(typename, field.name.value)) {
        invariant3.error(107, resultKeyNameFromField(field), result);
      }
    });
    try {
...
function warnAboutDataLoss(existingRef, incomingObj, storeFieldName, store) {
  ...
  __DEV__ && invariant3.warn(109, fieldName, parentType, childTypenames.length ? "either ensure all objects of type " + childTypenames.join(" and ") + " have an ID or a custom merge function, or " : "", typeDotName, __spreadValues({}, existing), __spreadValues({}, incoming));
}
...
var ObservableQuery = class {
  ...
  */
  ...
  constructor({
    ...
    if (options[variablesUnknownSymbol]) {
      invariant3(fetchPolicy === "standby", 77);
  ...
  */
  ...
  refetch(variables) {
    ...
    if (__DEV__ && variables && hasOwnProperty6.call(variables, "variables")) {
      ...
      if (!vars || !vars.some((v) => v.variable.name.value === "variables")) {
        __DEV__ && invariant3.warn(78, variables, queryDef.name?.value || queryDef);
      }
    }
  ...
  fetchMore({
    ...
    invariant3(this.options.fetchPolicy !== "cache-only", 79, getOperationName(this.query, "(anonymous)"));
    ...
    if (!pollInterval || !this.hasObservers() || fetchPolicy === "cache-only") {
      if (__DEV__) {
        if (!this.didWarnCacheOnlyPolling && pollInterval && fetchPolicy === "cache-only") {
          __DEV__ && invariant3.warn(82, getOperationName(this.query, "(anonymous)"));
...
function logMissingFieldErrors(missing) {
  if (__DEV__ && missing) {
    __DEV__ && invariant3.debug(83, missing);
  }
}
...
var QueryManager = class {
  ...
  mutate(_0) {
    return __async(this, arguments, function* ({
      ...
      if (hasClientExports) {
        if (__DEV__) {
          invariant3(this.localState, 85, getOperationName(mutation, "(anonymous)"));
        }
    ...
    if (__DEV__ && queryNamesAndQueryStrings.size) {
      queryNamesAndQueryStrings.forEach((included, nameOrQueryString) => {
        if (!included) {
          ...
          if (queryName) {
            __DEV__ && invariant3.warn(87, queryName);
          } else {
            __DEV__ && invariant3.warn(88);
          }
        }
      });
    }
  ...
  startGraphQLSubscription(options) {
    ...
    let restart;
    if (__DEV__) {
      invariant3(!this.getDocumentInfo(query).hasClientExports || this.localState, 89, getOperationName(query, "(anonymous)"));
    }
  ...
  getObservableFromLink(query, context, variables, extensions, deduplication = context?.queryDeduplication ?? this.queryDeduplication) {
    ...
    if (clientQuery) {
      ...
      if (__DEV__) {
        invariant3(this.localState, 90, operation[0].toUpperCase() + operation.slice(1), operationName ?? "(anonymous)");
      }
      invariant3(!hasIncrementalDirective, 91, operation[0].toUpperCase() + operation.slice(1), operationName ?? "(anonymous)");
  ...
  fetchObservableWithInfo(options, {
    ...
    if (this.getDocumentInfo(normalized.query).hasClientExports) {
      if (__DEV__) {
        invariant3(this.localState, 92, getOperationName(normalized.query, "(anonymous)"));
      }
  ...
  maskOperation(options) {
    ...
    if (__DEV__) {
      ...
      if (this.dataMasking && fetchPolicy === "no-cache" && !isFullyUnmaskedOperation(document) && !this.noCacheWarningsByCause.has(cause)) {
        ...
        __DEV__ && invariant3.warn(93, getOperationName(document, `Unnamed ${operationType ?? "operation"}`));
      }
    }
  ...
  fetchQueryByPolicy({
    ...
    const resultsFromCache = (diff, networkStatus) => {
      ...
      if (
        ...
        if (__DEV__) {
          invariant3(this.localState, 94, getOperationName(query, "(anonymous)"));
        }
        onCacheHit();
        return from(this.localState.execute({
          client: this.client,
...
var ApolloClient = class {
  ...
  __requestRaw(request) {
    return execute2(this.link, request, {
      client: this
    });
  }
  /**
...
//# sourceMappingURL=chunk-4NJESCOJ.js.map
