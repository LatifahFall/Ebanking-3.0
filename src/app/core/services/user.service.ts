...
export class UserService {
  ...
  getPreferences(userId: string): Observable<User['preferences'] | null> {
    const u = this.users.find(x => x.id === userId);
    return of(u ? u.preferences : null).pipe(delay(150));
  }

  // ME: PUT /me/{id}/preferences
  updatePreferences(userId: string, prefs: Partial<User['preferences']>): Observable<User['preferences'] | null> {
    const u = this.users.find(x => x.id === userId);
    if (!u) return of(null).pipe(delay(200));
    Object.assign(u.preferences, prefs);
    return of(u.preferences).pipe(delay(150));
  }
